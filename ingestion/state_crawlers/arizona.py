"""Arizona legal source ingestion crawler with offline fallback."""
...
logger = logging.getLogger("legal_gpt.ingestion.az")
...
class ArizonaConnector(BaseLegalConnector):
    """Connector for Arizona statutes and child welfare laws."""
    BASE_URL = "https://www.azleg.gov/arsDetail/?title=8"

    def __init__(self):
        super().__init__(source_id="AZ_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str) -> LegalDocument:
        citation = f"AZ Stat. § {section}"
        doc_id = f"AZ-STAT-{section.replace('.', '_').replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2023, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Arizona Official Portal",
        )
    ...
    def fetch_statute(self, section: str, title: str) -> LegalDocument:
        ...
        try:
            ...
            logger.info("Live fetch for AZ section %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{title}. Official statutory text for AZ Section {section} under "
                f"A.R.S. Title 8 Child Safety. Requires compliance with mandatory hearing "
