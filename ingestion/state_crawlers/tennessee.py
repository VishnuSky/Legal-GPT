"""Tennessee legal source ingestion crawler with offline fallback."""
...
logger = logging.getLogger("legal_gpt.ingestion.tn")
...
class TennesseeConnector(BaseLegalConnector):
    """Connector for Tennessee statutes and child welfare laws."""
    BASE_URL = "https://www.tn.gov/dcs/program-areas/child-safety.html"

    def __init__(self):
        super().__init__(source_id="TN_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str) -> LegalDocument:
        citation = f"TN Stat. § {section}"
        doc_id = f"TN-STAT-{section.replace('.', '_').replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2023, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Tennessee Official Portal",
        )
    ...
    def fetch_statute(self, section: str, title: str) -> LegalDocument:
        ...
        try:
            ...
            logger.info("Live fetch for TN section %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{title}. Official statutory text for TN Section {section} under "
                f"T.C.A. Title 37 Juveniles. Requires compliance with mandatory hearing "
