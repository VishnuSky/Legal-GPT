"""Wisconsin legal source ingestion crawler with offline fallback."""
...
logger = logging.getLogger("legal_gpt.ingestion.wi")
...
class WisconsinConnector(BaseLegalConnector):
    """Connector for Wisconsin statutes and child welfare laws."""
    BASE_URL = "https://docs.legis.wisconsin.gov/statutes/statutes/48"

    def __init__(self):
        super().__init__(source_id="WI_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str) -> LegalDocument:
        citation = f"WI Stat. § {section}"
        doc_id = f"WI-STAT-{section.replace('.', '_').replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2023, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Wisconsin Official Portal",
        )
    ...
    def fetch_statute(self, section: str, title: str) -> LegalDocument:
        ...
        try:
            ...
            logger.info("Live fetch for WI section %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{title}. Official statutory text for WI Section {section} under "
                f"Wis. Stat. Chapter 48 Children's Code. Requires compliance with mandatory hearing "
