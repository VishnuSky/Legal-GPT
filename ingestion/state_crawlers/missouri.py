"""Missouri legal source ingestion crawler with offline fallback."""
...
logger = logging.getLogger("legal_gpt.ingestion.mo")
...
class MissouriConnector(BaseLegalConnector):
    """Connector for Missouri statutes and child welfare laws."""
    BASE_URL = "https://revisor.mo.gov/main/OneChapter.aspx?chapter=210"

    def __init__(self):
        super().__init__(source_id="MO_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str) -> LegalDocument:
        citation = f"MO Stat. § {section}"
        doc_id = f"MO-STAT-{section.replace('.', '_').replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2023, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Missouri Official Portal",
        )
    ...
    def fetch_statute(self, section: str, title: str) -> LegalDocument:
        ...
        try:
            ...
            logger.info("Live fetch for MO section %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{title}. Official statutory text for MO Section {section} under "
                f"RSMo Chapter 210 Child Protection and Chapter 211 Juvenile Courts. Requires compliance with mandatory hearing "
