"""Minnesota legal source ingestion crawler with offline fallback."""
...
logger = logging.getLogger("legal_gpt.ingestion.mn")
...
class MinnesotaConnector(BaseLegalConnector):
    """Connector for Minnesota statutes and child welfare laws."""
    BASE_URL = "https://www.revisor.mn.gov/statutes/cite/260C"

    def __init__(self):
        super().__init__(source_id="MN_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str) -> LegalDocument:
        citation = f"MN Stat. § {section}"
        doc_id = f"MN-STAT-{section.replace('.', '_').replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2023, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Minnesota Official Portal",
        )
    ...
    def fetch_statute(self, section: str, title: str) -> LegalDocument:
        ...
        try:
            ...
            logger.info("Live fetch for MN section %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{title}. Official statutory text for MN Section {section} under "
                f"Minn. Stat. Chapter 260C Juvenile Safety and Placement. Requires compliance with mandatory hearing "
