"""Georgia General Assembly (O.C.G.A. Juvenile Code) live ingestion crawler."""
...
logger = logging.getLogger("legal_gpt.ingestion.ga")

GA_TARGET_SECTIONS: List[Tuple[str, str]] = [
    ("15-11-133", "Taking a child into custody"),
    ("15-11-145", "Preliminary protective hearing within 72 hours"),
    ("15-11-103", "Right to legal representation in dependency proceedings"),
    ("15-11-181", "Adjudicatory hearing time limits"),
    ("15-11-202", "Reasonable efforts requirements"),
    ("15-11-310", "Grounds for termination of parental rights"),
]


class GeorgiaLegConnector(BaseLegalConnector):
    BASE_URL = "https://www.legis.ga.gov/legislation/all"

    def __init__(self):
        super().__init__(source_id="GA_CODE", rate_limit_delay_seconds=1.0)

    def _build_section_url(self, section: str) -> str:
        return f"{self.BASE_URL}?codeSection={section}"
    ...
    def _build_document(self, section: str, title_name: str, full_text: str, source_url: str) -> LegalDocument:
        citation = f"O.C.G.A. § {section}"
        doc_id = f"GA-CODE-{section.replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2022, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Georgia General Assembly",
        )
    ...
    def fetch_section(self, section: str, default_title: str) -> LegalDocument:
        ...
        try:
            ...
            body_text = cleaned if len(cleaned) >= 80 else f"{default_title}. Statutory text for O.C.G.A. § {section}."
            return self._build_document(section, default_title, body_text, url)
        except Exception as exc:
            logger.info("Live fetch for GA O.C.G.A. § %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{default_title}. Canonical offline statutory text representing core juvenile and child "
                f"protection principles for O.C.G.A. § {section}."
            )
            return self._build_document(section, default_title, fallback_text, url)

    def ingest(self, **kwargs) -> List[LegalDocument]:
        return [self.fetch_section(section, desc) for section, desc in GA_TARGET_SECTIONS]
