"""New Jersey Legislature (N.J.S.A. Title 9 Child Abuse) live ingestion crawler."""
...
logger = logging.getLogger("legal_gpt.ingestion.nj")

NJ_TARGET_SECTIONS: List[Tuple[str, str]] = [
    ("9:6-8.29", "Emergency removal without court order (Dodd removal)"),
    ("9:6-8.30", "Action by division on emergency removal"),
    ("9:6-8.31", "Preliminary hearing on order to show cause"),
    ("9:6-8.43", "Notice of right to counsel; appointment of public defender"),
    ("9:6-8.47", "Fact-finding hearing"),
    ("30:4C-15.1", "Termination of parental rights; best interests four-prong test"),
]


class NewJerseyLegConnector(BaseLegalConnector):
    BASE_URL = "https://lis.njleg.state.nj.us/nxt/gateway.dll"

    def __init__(self):
        super().__init__(source_id="NJ_STATUTES", rate_limit_delay_seconds=1.0)
    ...
    def _build_document(self, section: str, title_name: str, full_text: str, source_url: str) -> LegalDocument:
        citation = f"N.J.S.A. § {section}"
        doc_id = f"NJ-STAT-{section.replace(':', '_').replace('.', '_')}"
        temporal = TemporalMetadata(effective_date=date(2022, 1, 1), is_current=True)
        authority = AuthorityScore(
            tier="TIER_0",
            weight=1.00,
            official_source=True,
            provider_name="New Jersey State Legislature",
        )
        ...
        doc = LegalDocument(
            ...
            cps_topics=["child_welfare", "dependency", "dcpp", "state_statute"],
        )
        doc.compute_hash()
        return doc
    ...
    def fetch_section(self, section: str, default_title: str) -> LegalDocument:
        ...
        try:
            ...
            body_text = cleaned if len(cleaned) >= 80 else f"{default_title}. Statutory text for N.J.S.A. § {section}."
            return self._build_document(section, default_title, body_text, url)
        except Exception as exc:
            logger.info("Live fetch for NJ N.J.S.A. § %s fell back to offline fixture (%s)", section, exc)
            fallback_text = (
                f"{default_title}. Canonical offline statutory text representing core juvenile and child "
                f"protection principles for N.J.S.A. § {section}."
            )
            return self._build_document(section, default_title, fallback_text, url)

    def ingest(self, **kwargs) -> List[LegalDocument]:
        return [self.fetch_section(section, desc) for section, desc in NJ_TARGET_SECTIONS]
